...
```bash
docker-compose -f config/docker/docker-compose.yml logs -f neurograph-api
```
...
Adjust in `config/docker/docker-compose.yml` under `deploy.resources`.
...
```bash
docker-compose -f config/docker/docker-compose.yml --profile monitoring up -d
```
...
docker-compose -f config/docker/docker-compose.yml --profile tracing up -d
```
...
```bash
# Start all profiles
docker-compose -f config/docker/docker-compose.yml --profile monitoring --profile tracing up -d
```
...
```bash
docker-compose -f config/docker/docker-compose.yml up -d --scale neurograph-api=3
```
...
**Check logs:**
```bash
docker-compose -f config/docker/docker-compose.yml logs neurograph-api
```
...
```bash
docker-compose -f config/docker/docker-compose.yml build --progress=plain
```
